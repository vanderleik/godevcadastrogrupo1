...
public class ValidacaoDeDatas {
	...
	/**
	 * Corresponde ao tempo máximo que um colaborador pode ser contratado pela
	 * empresa, baseando-se no tempo máximo que se espera que um ser humano viva
	 */
	 ...
	 * 
	 * Verifica se a data está de acordo com as normas de contratação (14 <= idade
	 * <= 120).
	 * 
	 * @param colaborador
	 * @return true, caso a data seja válida
	...
	public boolean validaDataDeNascimentoColaborador(Colaborador colaborador) {
		if (colaborador.getPessoa().getDataDeNascimento().until(LocalDate.now(),
				ChronoUnit.YEARS) >= IDADE_MINIMA_PARA_CONTRATO
				&& colaborador.getPessoa().getDataDeNascimento().until(LocalDate.now(),
						ChronoUnit.YEARS) <= IDADE_MAXIMA_PARA_PESSOA) {
			return true;
		}
		return false;
	}
	...
	/**
	 * Verifica se a data informada é condizente com uma data de admissão
	 * (0<=Tempo<=TEMPO_MAXIMO_PARA_ADMISSAO).
	 * 
	 * @param colaborador
	 * @return true, caso a data seja válida
	 */
	public boolean validaDataAdmissao(Colaborador colaborador) {
		if (colaborador.getDataAdmissao().until(LocalDate.now(), ChronoUnit.DAYS) >= 0 && colaborador.getDataAdmissao()
				.until(LocalDate.now(), ChronoUnit.YEARS) <= TEMPO_MAXIMO_PARA_ADMISSAO) {
			return true;
		}
		return false;
	}
